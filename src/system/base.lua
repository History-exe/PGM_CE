...
function FGBase:load(file,num,timeticks,dtype)
	...
	if num==0 or timeticks==0 then
		self:unloadImage()
		local image = CacheImageLoad(am_pack.res,file,dtype)
		if event[255] == 0 then
			self.image_p[self.image_index+1] = ImageClone(image)
		elseif event[255] == 1 then
			self.image_p[self.image_index+1] = ImageClone(image,1)
		end
		ImageFree(image)
		self.file = file
		self.effect_type = false
...
function PGColorButton:ctor(...)
	if arg[2]==nil then
		self.image_p = CacheImageLoad(arg[1],IMG_8888)
	else
		-- ImageLoad(pack,filename,dtype)
		self.image_p = CacheImageLoad(arg[1],arg[2],IMG_8888)
	end
	self.dx = 0
	self.dy = 0
	self.ispress = false
...
function PGColorButton:update()
	if self.image_p then
		ImageSetMask(self.image_p,MAKE_RGBA_8888(255,255,255,self.alpha))
	end
end
